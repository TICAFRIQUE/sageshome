...
@section('content')
                            ...
                            {{-- <div class="col-md-3">
                                ...
                                <select class="form-select" name="status">
                                    ...
                                    <option value="confirmed" {{ $filters['status'] == 'confirmed' ? 'selected' : '' }}>
                                        Confirmées</option>
                                    <option value="pending" {{ $filters['status'] == 'pending' ? 'selected' : '' }}>En
                                        attente</option>
    ...
    <div class="row d-none d-print-block mb-4">
        <div class="col-12 text-center">
            ...
            <p class="mb-0">Période : {{ \Carbon\Carbon::parse($filters['start_date'])->format('d/m/Y') }} au
                        ...
                        <div class="col-md-6 mb-3">
                         ...
                         <div class="col-md-6 mb-3">
                            <div class="border border-success rounded p-3 text-center bg-success-subtle">
                                <h4 class="text-success mb-1">{{ number_format($stats['total_revenue'], 0, ',', ' ') }}
                                    FCFA</h4>
                    ...
                    {{-- <div class="row mt-3">
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="border border-warning rounded p-3 text-center bg-warning-subtle">
                                <h4 class="text-warning mb-1">{{ number_format($stats['pending_revenue'], 0, ',', ' ') }}
                                    FCFA</h4>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0"><i class="ri-building-line me-1"></i> Performance par Résidence
                            </h5>
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($residenceStats as $stat)
                                        <tr class="residence-row {{ $filters['residence_id'] == $stat['residence']->id ? 'selected' : '' }}"
                                            ...
                                            <td class="text-end">
                                                <strong>{{ number_format($stat['total_revenue'], 0, ',', ' ') }}
                                                    FCFA</strong>
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                <tfoot class="table-active">
                                    <tr>
                                        ...
                                        <th class="text-end">{{ number_format($stats['total_revenue'], 0, ',', ' ') }}
                                            FCFA</th>
                                    </tr>
                                </tfoot>
                            </table>
                                ...
                                <tbody>
                                    @foreach ($bookings as $booking)
                                        <tr>
                                            ...
                                            <td class="text-end">{{ number_format($booking->total_amount, 0, ',', ' ') }}
                                                FCFA</td>
                                            <td class="text-center">
    ...
    @if ($paymentMethodStats->count() > 0)
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0"><i class="ri-money-dollar-circle-line me-1"></i> Revenus par Moyen de
                            Paiement</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($paymentMethodStats as $method => $data)
                                        @php
                                            $methodInfo = $paymentLabels[$method] ?? [
                                                'label' => ucfirst($method),
                                                'icon' => 'ri-questionnaire-line',
                                                'color' => 'dark',
                                            ];
                                            $percentage =
                                                $totalPayments > 0 ? ($data['total'] / $totalPayments) * 100 : 0;
                                        @endphp
                                        <tr>
                                            <td>
                                                <i
                                                    class="{{ $methodInfo['icon'] }} text-{{ $methodInfo['color'] }} me-2"></i>
                                                <strong>{{ $methodInfo['label'] }}</strong>
                                            </td>
                                            <td class="text-center">
                                                <span
                                                    class="badge bg-{{ $methodInfo['color'] }}">{{ $data['count'] }}</span>
                                            </td>
                                            <td class="text-end">
                                            ...
                                            {{-- <td class="text-center">
                                                <div class="progress" style="height: 20px;">
                                                    <div class="progress-bar bg-{{ $methodInfo['color'] }}"
                                ...
                                <tfoot class="table-active">
                                    <tr>
                                        <th>TOTAL</th>
                                        <th class="text-center">{{ $paymentMethodStats->sum('count') }}</th>
                                        <th class="text-end">{{ number_format($totalPayments, 0, ',', ' ') }} FCFA</th>
