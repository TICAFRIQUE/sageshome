        ...
        <form method="GET" action="{{ route('dashboard.bookings') }}" class="row g-3" id="filterForm">
            ...
            <div class="col-md-3">
                ...
                <select name="sort" id="sort" class="form-select form-select-modern">
                    <option value="created_at" {{ request('sort') === 'created_at' || !request('sort') ? 'selected' : '' }}>
                        📅 Date de création
                    </option>
            ...
            <div class="col-md-2">
                ...
                <select name="direction" id="direction" class="form-select form-select-modern">
                    <option value="desc" {{ request('direction') === 'desc' || !request('direction') ? 'selected' : '' }}>
                        📉 Décroissant
                    </option>
        ...
        @foreach($bookings as $booking)
            <div class="col-lg-6 col-xl-4 mb-4">
                <div class="booking-card dashboard-card h-100 position-relative">
                    ...
                    <!-- Image avec lazy loading et hover effect -->
                    ...
                    @else
                        <div class="card-img-top bg-gradient-light d-flex align-items-center justify-content-center booking-image" 
                             style="height: 200px;">
                    ...
                    <div class="card-body d-flex flex-column p-4">
                        <h5 class="card-title text-dark fw-bold mb-2">{{ $booking->residence->name }}</h5>
                        ...
                        <div class="mt-auto">
                            <div class="d-grid gap-2">
                                <a href="{{ route('dashboard.booking.show', $booking) }}" class="btn btn-primary btn-modern">
                                ...
                                @if($booking->status === 'pending' || $booking->status === 'confirmed')
                                    @if(\Carbon\Carbon::parse($booking->check_in)->isFuture())
                                        <form action="{{ route('dashboard.booking.cancel', $booking) }}" method="POST" 
                                            ...
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn btn-outline-danger btn-modern w-100">
        ...
        <div class="card-body text-center py-5">
            ...
            <h4 class="text-dark mb-3">
                @if(request('status') && request('status') !== 'all')
                    ...
                    Aucune réservation {{ request('status') === 'pending' ? 'en attente' : (request('status') === 'confirmed' ? 'confirmée' : (request('status') === 'completed' ? 'terminée' : 'annulée')) }} trouvée
                @else
...
/* Cartes de réservation optimisées */
.booking-card {
    border: none;
    border-radius: 20px;
    overflow: hidden;
...
.booking-card:hover .image-overlay {
    opacity: 1;
}
...
/* Amélioration des cards au hover */
.dashboard-card {
    transition: all 0.3s ease;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.cancel-form').forEach(form => {
        form.addEventListener('submit', function(e) {
            ...
            const bookingName = this.closest('.booking-card').querySelector('.card-title').textContent;
